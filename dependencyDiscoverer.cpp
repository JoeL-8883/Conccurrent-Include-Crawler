...
struct Table {
  private:
    ...
    std::mutex mutex;
  public:
    std::unordered_map<std::string, std::list<std::string>> getMap() {
      return table;
    }

    std::list<std::string> *getTablePointer(std::string name) {
      std::unique_lock<std::mutex> lock(mutex);
      return &table[name];
    }

    void add_concurrent(std::string file, std::list<std::string> dependenceies) {
      std::unique_lock<std::mutex> lock(mutex);
      table.insert({file, dependenceies});
    }

    bool find_concurrent(std::string target) {
      std::unique_lock<std::mutex> lock(mutex);
      
      // Check if file is in the hash table
      if (auto search = table.find(target); search != table.end()) {
        return true;
      } else {
        return false;
      }
...
struct WorkQ {
  ...
  public:
    ...
    void append_concurrent(std::string s) {
      std::unique_lock<std::mutex> lock(mutex);
      list.push_front(s);
    }
    
    void removeFront_concurrent() {
      std::unique_lock<std::mutex> lock(mutex);
      list.pop_front();
    }

    std::string front_concurrent() {
      std::unique_lock<std::mutex> lock(mutex);
      return list.front();
    }

    std::string end_concurrent() {
      std::unique_lock<std::mutex> lock(mutex);
      return *list.end();
    }
...
static void process(const char *file, std::list<std::string> *ll) {
  ...
  while (fgets(buf, sizeof(buf), fd) != NULL) {
    ...
    *q = '\0';
    ...
    if (newTable.find_concurrent(name)) { continue; }  
    // ... insert mapping from file name to empty list in table ...
    newTable.add_concurrent(name, {});
    // ... append file name to workQ
    newQueue.append_concurrent(name);
  }
  // 3. close file
  fclose(fd);
}
...
static void printDependencies(std::unordered_set<std::string> *printed,
  ...
  while ( toProcess->size() > 0 ) {
    ...
    std::list<std::string> *ll = newTable.getTablePointer(name);
...
int main(int argc, char *argv[]) {
  ...
  for (i = start; i < argc; i++) {
    ...
    newTable.add_concurrent(obj, {argv[i]});
    
    // 3b. insert mapping from file.ext to empty list
    newTable.add_concurrent(argv[i], {});
    
    // 3c. append file.ext on workQ
    newQueue.append_concurrent(argv[i]);
  }

  // 4. for each file on the workQ
  while ( newQueue.size_concurrent() > 0 ) {
    std::string filename = newQueue.front_concurrent();
    newQueue.removeFront_concurrent();

    if (!newTable.find_concurrent(filename)) {
      fprintf(stderr, "Mismatch between table and workQ\n");
      return -1;
    }

    // 4a&b. lookup dependencies and invoke 'process'
    //auto first = std::thread(process(filename.c_str(), newTable.getMap()[filename]));
    process(filename.c_str(), newTable.getTablePointer(filename));
    //first.join();
  }
